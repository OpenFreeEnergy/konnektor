...
from konnektor.utils.toy_data import genMapper, genScorer, build_random_dataset, build_random_mst_network, \
...
def test_build_n_random_mst_network(n_sub_networks, overlap):
    n_compounds = 30
    mst_networks = build_n_random_mst_network(n_compounds=n_compounds, sub_networks=n_sub_networks, overlap=overlap, rand_seed=42)
    ...
    if overlap > 0:
        for i, net in enumerate(mst_networks):
            if i == 0:
                ...
                assert len(net.nodes) ==  n_compounds//n_sub_networks + n_compounds%n_sub_networks + overlap
            else:
