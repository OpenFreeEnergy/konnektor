...
def test_star_network(atom_mapping_basic_test_files, toluene_vs_others,
    ...
    network = planner.generate_ligand_network(components=others,
                                              central_component=toluene)
    ...
    assert all((central_ligand_name in {mapping.componentA.name,
...
def test_star_network_with_scorer(toluene_vs_others):
    ...
    for edge in network.edges:
        assert len(
            edge.componentA_to_componentB) > 1  # we didn't take the bad mapper
        assert 'score' in edge.annotations
        assert edge.annotations['score'] == 1.0 / len(
...
def test_star_network_failure(atom_mapping_basic_test_files):
    ...
    with pytest.raises(ValueError, match='No mapping found for'):
        network = planner.generate_ligand_network(components=[nigel],
                                                  central_component=
