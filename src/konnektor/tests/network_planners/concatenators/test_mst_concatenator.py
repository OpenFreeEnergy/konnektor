...
def test_score_all_inter_network_edges_respects_exclusions():
    ...
    mappings = concatenator._score_all_inter_network_edges(networkA, networkB, exclude=set())
    excluded = frozenset((mappings[0].componentA, mappings[0].componentB))

    # Re-run with that mapping excluded.
    mappings = concatenator._score_all_inter_network_edges(networkA, networkB, exclude={excluded})
...
def test_tied_scores_pick_highest_key():
    ...
    candidates = concatenator._score_all_inter_network_edges(networkA, networkB, exclude=set())
