...
class CyclicNetworkAlgorithm(_AbstractNetworkAlgorithm):
    ...
    def generate_network(self, edges: list[tuple[int, int]],
        ...
        self._cyclic_paths_scored = self._generate_cyclic_paths(
            graph=self.orig_g)
        ...
        log.info("TIMINGS:")
        log.info(
            "\t Cycle generation duration: " + str(duration_path_generation))
        log.info(
            "\t Cycle selection duration: " + str(duration_cycle_selection))
    ...
    def _translate_input(self, edges: List[Tuple[int, int]],
                         weights: List[float]) -> Graph:
    ...
    def _generate_cyclic_paths(self, graph: Graph) -> Tuple[List[int], float]:
        ...
        if (self.max_sub_cycle_size > n_nodes):
            raise ValueError(
                "a cycle can not be larger, than the number of nodes.")

        # Build Paths
        log.info(
            "\tGenerate all paths up to size " + str(self.max_sub_cycle_size))
        raw_cycles = [c for c in nx.simple_cycles(graph,
                                                  length_bound=self.max_sub_cycle_size)]
        log.info("\tFound  " + str(len(raw_cycles)) + " cycles")

        # Filter cycle sizes:
        log.info("\tFilter too small cycles")
        selected_cycle_by_size = set([tuple(x) for x in raw_cycles if
                                      (len(x) in self.sub_cycle_size_range)])
    ...
    def _greedy_select_cylces_by_node_connectivity(self,
                                                   cyclic_paths_scored: Tuple[
                                                       List[int], float],
                                                   graph: Graph) -> \
            List[List[int]]:
        # Build priorityQueue for MST selection
        log.info("\tBuild Cycle Priority Queue")
        cycle_priority_queue = list(map(lambda x: x[0],
                                        sorted(cyclic_paths_scored,
                                               key=lambda x: x[1])))

        # Greedy opt algorithm: optimize connectivity criteria + select cycles Kruskal-MST like
        log.info("\tOptimize such that each node appears in " + str(
            self.node_cycle_connectivity) + " Cycles")
        ...
        while not termination_criteria(cycle_connectivity_dict):
            ...
            if (any([cycle_connectivity_dict[n] < self.node_cycle_connectivity
                     for n in
    ...
    def generate_network_double_greedy(self, edges: List[Tuple[int, int]],
        ...
        log.info(
            "Priority Queue Gen init - " + str(start_time_cycle_generation))
        ...
        for target_node, prioqueue in min_node.items():
            not_target_node = lambda e: not (
                        e[0] == target_node or e[1] == target_node)
            ...
            for i in range(self.node_cycle_connectivity):
                ...
                adding_node = lambda e: len(
                    cycle_nodes.intersection(set(e))) == 1
                use_edge = lambda e: not e in cycle_edges and adding_node(
                    e) and not_target_node(e)

                successor_node = prioqueue[i][1] if (
                            target_node == prioqueue[i][0]) else prioqueue[i][0]
                for j in range(self.max_sub_cycle_size - 2):
                    ...
                    successor_node = min_e[1] if (
                                successor_node == min_e[0]) else min_e[0]

                if (len(cycle_edges) == self.max_sub_cycle_size - 1):
                    cycle_edges.append(
                        tuple(sorted([successor_node, target_node])))
        ...
        log.info("TIMINGS:")
        log.info(
            "\t Cycle generation duration: " + str(duration_cycle_generation))
        log.info(
            "\t Cycle selection duration: " + str(duration_cycle_selection))
        log.info("\t total duration: " + str(duration_total))
