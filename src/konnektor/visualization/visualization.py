...
def draw_ligand_network(network: LigandNetwork, title:str="", ax:plt.Axes=None,
                        node_size:int=2050, edge_width:int=3, fontsize:int=18)->plt.Figure:
    ...
    mixins = np.clip(
        connectivities / (sum(connectivities) / len(connectivities)), a_min=0,
        a_max=2) / 2

    cs = list(map(lambda x: color_gradient(mix=x, hex=False), mixins))

    nx.draw_networkx(g, pos=pos, with_labels=True, ax=ax, node_size=node_size,
                     width=edge_width,
                     node_color=cs, edge_color=OFE_COLORS[3],
                     font_color=[1, 1, 1])
